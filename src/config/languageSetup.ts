...
export async function handleFirstTimeLanguageSetup(): Promise<void> {
  ...
  if (firstRun) {
    ...
    if (selectedLanguage === 'de') {
      ...
      console.log(
        chalk.green(`✅ Language set to 🇺🇸 English! WOARU is now ready.`)
      );
    }
...
export async function showLanguageStatus(): Promise<void> {
  ...
  console.log(
    `   Current: ${LANGUAGE_NAMES[currentLang as SupportedLanguage] || currentLang} (${currentLang})`
  );
  console.log(
    `   Available: ${SUPPORTED_LANGUAGES.map(lang => `${LANGUAGE_NAMES[lang]} (${lang})`).join(', ')}`
  );
  console.log(
    chalk.gray(
