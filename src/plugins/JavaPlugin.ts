...
export class JavaPlugin extends BasePlugin {
  ...
  getRecommendations(analysis: ProjectAnalysis): SetupRecommendation[] {
    ...
    if (!this.hasPackage(analysis, 'checkstyle')) {
      recommendations.push({
        ...
        category: 'code-style',
        reason:
          'Enforce consistent Java coding standards and catch style violations',
        packages: ['checkstyle'],
  ...
  getRefactorSuggestions(analysis: ProjectAnalysis): RefactorSuggestion[] {
    ...
    javaFiles.forEach(file => {
      ...
      if (
        analysis.framework.includes('spring') ||
        ...
        if (file.includes('Controller.java')) {
          suggestions.push({
            filename: file,
            suggestion:
              'Use @RestController instead of @Controller + @ResponseBody for REST APIs',
            type: 'best-practice',
          });
        }
      ...
      if (file.includes('Test.java') || file.includes('Tests.java')) {
        suggestions.push({
          filename: file,
          suggestion:
            'Use @ParameterizedTest for testing multiple inputs efficiently',
      ...
      if (file.includes('Main.java') || file.includes('Application.java')) {
        suggestions.push({
          filename: file,
          suggestion:
            'Consider using dependency injection instead of static dependencies',
          type: 'maintainability',
        });
      }
    });
