...
export class EslintPlugin extends HybridBasePlugin {
  ...
  async runLinter(
    ...
    try {
      ...
      const eslintConfig =
        await this.databaseManager.getCoreToolConfig('eslint');

      if (!eslintConfig) {
        console.warn(
          '⚠️ ESLint configuration not found in database, using defaults'
        );
        return this.runEslintDefault(filePath, options);
      }

      // Build command with recommended flags from database
      const args = this.buildEslintCommand(
        filePath,
        eslintConfig.recommended_flags,
        options
      );
  ...
  async checkDeprecationWarnings(): Promise<{
    ...
    try {
      ...
      const deprecationInfo =
        await this.databaseManager.getDeprecationInfo('eslint');
      ...
      const tslintDeprecation =
        await this.databaseManager.getDeprecationInfo('tslint');
  ...
  async getRecommendedPackages(projectContext: {
    ...
    try {
      ...
      if (projectContext.framework) {
        const frameworkRecs =
          await this.databaseManager.getFrameworkRecommendations(
      ...
      console.warn(
        'Failed to get recommended ESLint packages from database:',
        error
      );
      return this.getDefaultPackages(projectContext);
    }
   ...
   */
  private async runEslintDefault(
    filePath: string,
    options: { fix?: boolean }
  ): Promise<{
    success: boolean;
