...
export class TemplateRegistry implements ITemplateRegistry {
  ...
  register(template: ProjectTemplate): void {
    ...
    if (!validation.valid) {
      throw new Error(
        `Invalid template "${template.id}": ${validation.errors.map(e => e.message).join(', ')}`
      );
    }
    ...
    const existingIndex = categoryTemplates.findIndex(
      t => t.id === template.id
    );
  ...
  validate(template: ProjectTemplate): ValidationResult {
    ...
    if (
      ...
      errors.push({
        ...
        message: 'Template must have structure with directories and files',
      });
    }
    ...
    if (template.features) {
      template.features.forEach((feature, index) => {
        ...
        if (feature.dependencies && feature.conflicts) {
          const conflictingDeps = feature.dependencies.filter(dep =>
            feature.conflicts?.includes(dep)
          );
    ...
    if (
      ...
      template.dependencies.runtime.length === 0
    ) {
      warnings.push({
        code: 'NO_RUNTIME_DEPS',
        message:
          'Template has no runtime dependencies - this might be intentional',
      });
    }
    ...
    if (!template.features || template.features.length === 0) {
      warnings.push({
        code: 'NO_FEATURES',
        message:
          'Template has no optional features - consider adding some for flexibility',
      });
    }
  ...
  private async loadTemplates(): Promise<void> {
    try {
      if (!(await fs.pathExists(this.templatesPath))) {
        console.warn(
          chalk.yellow(
            `⚠️ Templates directory not found: ${this.templatesPath}`
          )
        );
        this.loadBuiltinTemplates();
        return;
      ...
      for (const templateDir of templateDirs) {
        ...
        if (await fs.pathExists(templateFilePath)) {
          try {
            ...
            console.warn(
              chalk.yellow(
                `⚠️ Failed to load template from ${templatePath}:`,
                error
              )
            );
          }
      ...
      console.warn(
        chalk.yellow(
          '⚠️ Failed to load additional templates from filesystem, using builtin templates only'
        )
      );
    }
  }
  ...
  private loadBuiltinTemplates(): void {
    ...
    const nextjsTemplate: ProjectTemplate = {
      ...
      description:
        'Modern React application with Next.js, TypeScript, and Tailwind CSS',
      category: 'frontend',
      language: 'TypeScript',
      ...
      structure: {
        ...
        templates: [
          {
            source: 'nextjs/package.json.hbs',
            destination: 'package.json',
            variables: {},
          },
          {
            source: 'nextjs/tsconfig.json',
            destination: 'tsconfig.json',
            variables: {},
          },
          {
            source: 'nextjs/next.config.js.hbs',
    ...
    const fastapiTemplate: ProjectTemplate = {
      ...
      structure: {
        ...
        templates: [
          {
            source: 'python-fastapi/main.py.hbs',
            destination: 'src/main.py',
            variables: {},
          },
          {
            source: 'python-fastapi/requirements.txt.hbs',
            destination: 'requirements.txt',
            variables: {},
          },
          {
            source: 'python-fastapi/pyproject.toml.hbs',
      ...
      configuration: {
        ...
        '.pre-commit-config.yaml': {
          template: 'python-fastapi/.pre-commit-config.yaml',
        },
      },
      features: [
        {
