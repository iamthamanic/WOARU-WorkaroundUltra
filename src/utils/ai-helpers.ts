...
export async function ensureAiIsConfigured(): Promise<void> {
  try {
    ...
    const enabledProviders = Object.entries(aiConfig).filter(
      ([key, value]: [string, any]) =>
        key !== 'lastDataUpdate' && value && value.enabled === true
    );
    ...
    console.error(
      chalk.red(`❌ ${i18next.t('ai_helpers.config_check_error')}`),
      error
    );
    showAiConfigurationError();
    process.exit(1);
...
function showAiConfigurationError(): void {
  console.error(
    chalk.red(`❌ ${i18next.t('ai_helpers.config_missing_error')}`)
  );
 ...
 */
export async function getActiveAiProviders(): Promise<
  Array<{ id: string; provider: any }>
> {
  try {
    const configManager = ConfigManager.getInstance();
    const aiConfig = await configManager.loadAiConfig();
    ...
    console.warn(
      chalk.yellow(`⚠️ ${i18next.t('ai_helpers.config_loading_error')}`),
      error
    );
    return [];
  }
