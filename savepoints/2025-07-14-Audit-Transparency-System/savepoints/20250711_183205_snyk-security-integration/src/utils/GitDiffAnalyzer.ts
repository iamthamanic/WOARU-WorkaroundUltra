...
export class GitDiffAnalyzer {
  ...
  async getChangedFilesSince(baseBranch: string = 'main'): Promise<GitDiffResult> {
  ...
  categorizeChangedFiles(files: string[]): {
    ...
    files.forEach(file => {
      ...
      // Documentation
      else if (['.md', '.txt', '.rst'].includes(ext) || relativePath.includes('docs')) {
        result.docs.push(file);
      }
      // Source files
      else if (['.js', '.jsx', '.ts', '.tsx', '.py', '.rs', '.go', '.java', '.cs', '.php', '.rb'].includes(ext)) {
