...
export class WOARUSupervisor extends EventEmitter {
  ...
  private async handleFileChanges(changes: FileChange[]): Promise<void> {
    ...
    const packageFiles = ['package.json', 'package-lock.json', 'pyproject.toml', 'requirements.txt', 'Gemfile', 'go.mod'];
    ...
    // Run security check asynchronously if package files changed
    if (hasPackageChanges) {
      this.runBackgroundSecurityCheck().catch(error => {
  ...
  /**
   * Runs Snyk security check in the background when package files change
   */
  private async runBackgroundSecurityCheck(): Promise<void> {
    this.notificationManager.showProgress('🔒 Running security scan on updated dependencies...');

    try {
      // Get list of all files for security check (though Snyk will scan the whole project)
      const allFiles = await this.getAllProjectFiles();
      
      // Run Snyk checks
      const snykResults = await this.qualityRunner.runSnykChecks(allFiles);
      ...
      snykResults.forEach(result => {
        ...
        // Show specific critical vulnerabilities
        if (result.type === 'dependencies' && result.vulnerabilities) {
          ...
          criticalVulns.forEach(vuln => {
            this.notificationManager.showSecurityAlert(
              'critical',
              `CRITICAL: ${vuln.title}`,
              `Package: ${vuln.packageName}@${vuln.version}${vuln.isUpgradable ? ' (Fix available)' : ''}`,
              vuln.isUpgradable ? `Run: npm update ${vuln.packageName}` : undefined
            );
          });

          // Show high vulnerabilities in a summarized way
          if (highVulns.length > 0 && highVulns.length <= 3) {
            highVulns.forEach(vuln => {
              this.notificationManager.showSecurityAlert(
                'high',
                `HIGH: ${vuln.title}`,
                `Package: ${vuln.packageName}@${vuln.version}`,
                vuln.isUpgradable ? `Fix available` : undefined
              );
            });
            ...
            this.notificationManager.showSecurityAlert(
              ...
              'Run "snyk test" for details',
              'Consider updating vulnerable packages'
            );
          }
        }

        // Show code security issues
        if (result.type === 'code' && result.codeIssues) {
          ...
          if (criticalCodeIssues.length > 0) {
            criticalCodeIssues.slice(0, 3).forEach(issue => {
              this.notificationManager.showSecurityAlert(
                issue.severity as 'critical' | 'high',
                `Code Security: ${issue.title}`,
                `${issue.filePath}:${issue.line}`,
                'Review and fix the security issue'
              );
            });

            if (criticalCodeIssues.length > 3) {
              this.notificationManager.showSecurityAlert(
                'high',
                `${criticalCodeIssues.length - 3} more security issues found`,
                'Run "snyk code test" for full report'
              );
            }
          }
        }
      ...
      if (!hasCriticalIssues) {
        this.notificationManager.showSuccess('✅ Security scan complete - no critical vulnerabilities found');
      } else {
        // Show summary alert
        this.notificationManager.showSecurityAlert(
          'critical',
          `SECURITY ALERT: ${criticalCount} critical and ${highCount} high vulnerabilities found!`,
          'Your dependencies have known security issues',
          'Run "snyk test" for detailed report or "snyk wizard" for guided fixes'
        );
      }
  ...
  /**
   * Helper to get all project files (for Snyk Code analysis)
   */
  private async getAllProjectFiles(): Promise<string[]> {
    // For now, return empty array as Snyk scans the whole project anyway
    // This could be enhanced to return actual file list if needed
    return [];
  }
}
