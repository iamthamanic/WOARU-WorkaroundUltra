...
export class NotificationManager extends EventEmitter {
  ...
  showSecurityAlert(
    ...
    if (this.config.desktop && severity === 'critical') {
      // Check cooldown
      const lastSecurityNotification = this.notificationHistory.get('security_alert');
      if (!lastSecurityNotification || 
          (Date.now() - lastSecurityNotification.getTime()) / 1000 / 60 >= this.cooldownMinutes) {
