  ...
  .action(async options => {
    try {
      ...
      if (result.security_summary && !options.noSecurity) {
        ...
        const healthColor = 
          security.health_score >= 80 ? chalk.green : 
          security.health_score >= 60 ? chalk.yellow : chalk.red;

        console.log(chalk.cyan.bold('🔒 SECURITY ANALYSIS'));
        console.log(chalk.gray('─'.repeat(40)));
        console.log(healthColor(`Security Health Score: ${security.health_score}/100`));
      ...
      if (result.production_audits && result.production_audits.length > 0) {
        ...
        const auditsByCategory = result.production_audits.reduce((acc: any, audit: any) => {
          if (!acc[audit.category]) acc[audit.category] = [];
          acc[audit.category].push(audit);
          return acc;
        }, {});

        Object.entries(auditsByCategory).forEach(([category, audits]: [string, any]) => {
          console.log(chalk.yellow(`📋 ${category.toUpperCase()}`));
          audits.forEach((audit: any) => {
            const priorityColor = 
              audit.priority === 'critical' ? chalk.red :
              audit.priority === 'high' ? chalk.yellow :
              audit.priority === 'medium' ? chalk.blue : chalk.gray;
            
            console.log(priorityColor(`   • ${audit.message}`));
      ...
      if (result.code_insights && result.code_insights.length > 0) {
        ...
        result.code_insights.forEach((insight: any) => {
          const severityColor = 
            insight.severity === 'critical' ? chalk.red :
            insight.severity === 'high' ? chalk.yellow : chalk.blue;
          
          console.log(severityColor(`   ${insight.tool}: ${insight.reason}`));
          if (insight.evidence && insight.evidence.length > 0) {
            console.log(chalk.gray(`     Evidence: ${insight.evidence.slice(0, 3).join(', ')}${insight.evidence.length > 3 ? '...' : ''}`));
          }
        });
        console.log();
      }
      ...
      // Summary
      const totalIssues = (result.security_summary?.total_issues || 0) + (result.production_audits?.length || 0);
      if (totalIssues > 0) {
        console.log(chalk.yellow(`💡 Found ${totalIssues} total issues. Run "woaru setup" to address tool recommendations.`));
        if (result.security_summary && result.security_summary.total_issues > 0) {
          console.log(chalk.red(`🔒 Security issues require immediate attention. Review output above.`));
        }
      } else {
        console.log(chalk.green('🎉 Project looks great! No critical issues found.'));
      }
