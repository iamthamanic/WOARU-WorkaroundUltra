...
export class SOLIDChecker {
   ...
   */
  async analyzeFile(filePath: string, language: string): Promise<SOLIDCheckResult> {
    ...
    for (const [principle, checker] of this.checkers) {
      if (checker.supportsLanguage(language)) {
        try {
          ...
          console.warn(`SOLID-Checker: Fehler beim ${principle}-Check für ${filePath}:`, error);
        }
      }
    }
   ...
   */
  async analyzeProject(filePaths: string[], language: string): Promise<ProjectSOLIDResult> {
    ...
    const recommendations = this.generateProjectRecommendations(allViolations, fileResults);
   ...
   */
  private async calculateFileMetrics(filePath: string, violations: SOLIDViolation[]) {
    // Extrahiere Metriken aus Violations
    const classCount = new Set(violations.map(v => v.class).filter(Boolean)).size;
  ...
  private calculateOverallSOLIDScore(fileResults: SOLIDCheckResult[]): number {
    ...
    const totalScore = fileResults.reduce((sum, result) => sum + result.metrics.solidScore, 0);
  ...
  private groupViolationsByPrinciple(violations: SOLIDViolation[]) {
    ...
    principles.forEach(principle => {
      ...
      const score = principleViolations.length === 0 ? 100 : 
  ...
  private generateFileSuggestions(violations: SOLIDViolation[]): string[] {
      ...
      switch (violation.principle) {
        case 'SRP':
          if (violation.severity === 'critical' || violation.severity === 'high') {
            suggestions.add('🔄 Refaktoriere große Klassen in kleinere, fokussierte Einheiten');
            suggestions.add('📦 Separiere verschiedene Concerns in eigene Services');
          }
          if (violation.metrics?.methodCount && violation.metrics.methodCount > 20) {
            suggestions.add('🧩 Verwende das Facade Pattern um komplexe Klassen zu vereinfachen');
          }
          break;
      }
    });
    ...
    if (suggestions.size === 0 && violations.length > 0) {
      suggestions.add('📚 Überprüfe die SOLID-Prinzipien Dokumentation für Verbesserungsideen');
    }
  ...
  private generateProjectRecommendations(
    ...
    if (violationsByPrinciple.SRP.violations > 0) {
      ...
      const avgScore = fileResults.reduce((sum, r) => sum + r.metrics.solidScore, 0) / fileResults.length;
      if (avgScore < 70) {
        recommendations.push('🏗️ Führe systematisches Refactoring durch - beginne mit den kritischsten Verstößen');
      }
    }

    // Allgemeine Architektur-Empfehlungen
    const filesWithManyViolations = fileResults.filter(r => r.violations.length > 5);
